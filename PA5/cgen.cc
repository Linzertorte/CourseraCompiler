...
static CgenClassTable *cgen_ctx = NULL;
...
void program_class::cgen(ostream &os) {
    ...
    cgen_ctx = new CgenClassTable(classes, os);
...
static void emit_method_intro(ostream &str) {
...
// This search will get all the methods for classes and all the attrs for classes

void CgenClassTable::dfs(CgenNodeP root) {
    class_tag.add_tag(root->get_name());
    if (root->get_name() == Object) {
        method_table.inherit_methods_from_parent(root->get_name(), NULL);
        attr_table.inherit_attrs_from_parent(root->get_name(), NULL);
    } else {
        method_table.inherit_methods_from_parent(root->get_name(), root->get_parent());
        attr_table.inherit_attrs_from_parent(root->get_name(), root->get_parent());
    }
    for (int i = root->features->first(); root->features->more(i); i = root->features->next(i)) {
        method_class *m = dynamic_cast<method_class *> (root->features->nth(i));
        if (m) method_table.add_method(root->get_name(), m->name);
        attr_class *attr = dynamic_cast<attr_class *> (root->features->nth(i));
        if (attr) attr_table.add_attr(root->get_name(), attr->name, attr->type_decl);
    }
...
void CgenClassTable::code_disp_table() {
    method_table.code_disp_table(str);
}

void CgenClassTable::code_class_name_table() {
    str << "class_nameTab" << LABEL;
    for (size_t i = 0; i < class_tag.tbl.size(); i++) {
        str << WORD;
        StringEntryP e = stringtable.add_string(((StringEntryP) (class_tag.tbl[i]))->get_string());
        e->code_ref(str);
        str << endl;
    }
}
...
void CgenClassTable::code_class_obj_table() {
    ...
    for (size_t i = 0; i < class_tag.tbl.size(); i++) {
        str << WORD;
        str << class_tag.tbl[i] << "_protObj";
        str << endl;
        str << WORD;
        str << class_tag.tbl[i] << "_init";
        str << endl;
    }
}

void CgenClassTable::code_default_value_for_type(Symbol t) {
    if (t == Bool) falsebool.code_ref(str);
    else if (t == Str) ((StringEntryP) (stringtable.add_string("")))->code_ref(str);
    else if (t == Int) ((IntEntryP) (inttable.add_string("0")))->code_ref(str);
    else str << "0";
...
void CgenClassTable::code_prot_obj() {
    ...
    for (i = attr_table.tbl.begin(); i != attr_table.tbl.end(); i++) {
        str << WORD << -1 << endl;
        str << i->first << "_protObj" << LABEL;
        str << WORD << class_tag.class_to_tag(i->first) << endl;
        str << WORD << i->second.size() + 3 << endl;
        str << WORD << i->first << "_dispTab" << endl;
        if (i->first == Bool || i->first == Int) {
            str << WORD << "0" << endl;
        } else if (i->first == Str) {
            str << WORD;
            ((IntEntryP) (inttable.add_string("0")))->code_ref(str);
            str << endl;
            str << WORD << "0" << endl;
        } else {
            for (size_t j = 0; j < i->second.size(); j++) {
                str << WORD;
                code_default_value_for_type((i->second)[j].second);
                str << endl;
            }
        }
    }
...
void CgenClassTable::code_methods_dfs(CgenNodeP root) {
    ...
    for (int i = root->features->first(); root->features->more(i); i = root->features->next(i)) {
        if (root->get_name() != Object && root->get_name() != IO && root->get_name() != Str) {
            cur_symtab.init((attr_table.tbl)[root->get_name()]);
            method_class *m = dynamic_cast<method_class *> (root->features->nth(i));
            if (m) {
